...
-- =====================================================
-- GENERIC NOTIFICATION TRIGGER SETUP
-- =====================================================

-- Drop any existing triggers and functions from the old setup
DROP TRIGGER IF EXISTS update_service_configs_timestamp ON service_configs;
DROP TRIGGER IF EXISTS trigger_check_signals ON whale_trades;
...
DROP FUNCTION IF EXISTS update_service_config_timestamp();
DROP FUNCTION IF EXISTS check_for_signals();
DROP FUNCTION IF EXISTS notify_signal_webhook();
DROP FUNCTION IF EXISTS notify_trade_webhook();
DROP FUNCTION IF EXISTS test_webhook();

-- Create the generic table change notification function
CREATE OR REPLACE FUNCTION notify_table_change()
RETURNS TRIGGER AS $func$
DECLARE
  data_payload jsonb;
  old_data_payload jsonb;
  return_record RECORD;
BEGIN
  -- Determine data payload based on operation
  ...
  -- Determine old data payload for updates
  IF TG_OP = 'UPDATE' THEN
    old_data_payload := row_to_json(OLD);
  ELSE
    old_data_payload := NULL;
  END IF;
...
-- Attach the generic trigger to all tables
CREATE TRIGGER tracked_wallets_change_trigger
...
CREATE TRIGGER whale_trades_change_trigger
  AFTER INSERT OR UPDATE OR DELETE ON whale_trades
  FOR EACH ROW EXECUTE FUNCTION notify_table_change();

CREATE TRIGGER trade_signals_change_trigger
  AFTER INSERT OR UPDATE OR DELETE ON trade_signals
  FOR EACH ROW EXECUTE FUNCTION notify_table_change();

CREATE TRIGGER portfolio_trades_change_trigger
  AFTER INSERT OR UPDATE OR DELETE ON portfolio_trades
  FOR EACH ROW EXECUTE FUNCTION notify_table_change();

CREATE TRIGGER signal_config_change_trigger
  AFTER INSERT OR UPDATE OR DELETE ON signal_config
  FOR EACH ROW EXECUTE FUNCTION notify_table_change();

CREATE TRIGGER service_configs_change_trigger
  AFTER INSERT OR UPDATE OR DELETE ON service_configs
  FOR EACH ROW EXECUTE FUNCTION notify_table_change();

CREATE TRIGGER service_heartbeats_change_trigger
  AFTER INSERT OR UPDATE OR DELETE ON service_heartbeats
  FOR EACH ROW EXECUTE FUNCTION notify_table_change();
